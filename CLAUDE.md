...
CI에서도 `e2e` job(`.github/workflows/ci.yml`)이 동일 절차를 PR마다 실행한다. Playwright 브라우저는 `@playwright/test` 버전을 키로 캐싱되어 매 실행마다 다운로드되지 않는다. PR 머지 게이트로 묶고 싶으면 branch protection에서 `e2e` 체크를 required로 추가한다 (job 이름은 안정적으로 유지).
