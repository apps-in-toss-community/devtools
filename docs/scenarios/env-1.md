...
- 토스 WebView native bridge: 환경 3·4(`devtools-mcp` + `start_debug(relay-dev/relay-live)`)로 보완

다음 단계: 실기기 검증이 필요하면 `npx @ait-co/devtools devtools-mcp` 실행 후 `start_debug({mode: 'relay-dev'})` → `get_diagnostics`로 상태 확인.
