...
> `--target=local`로 기동했어도 `start_debug(relay-live)`로 live로 hot-switch 가능하다(#356 DualConnectionRouter 대칭화).
> local-browser 계열로 전환하면 LIVE guard가 자동 disarm된다.
...
2. **`measure_safe_area`**
   - `source: "relay-live"` (출력 env.kind 불변 — 입력 mode `relay-live`에서도 동일)
...
`relay-live` 모드 세션(`start_debug({mode: 'relay-live', confirm: true})` 이후)에서 `call_sdk` 또는 `evaluate`를 호출하면 **명시적 동의 인자(`confirm: true`)가 없을 때 거부된다**.
...
- 환경 3(`relay-staging`)은 `confirm` 없이 자유롭게 `call_sdk` 호출 가능. 환경 4(`relay-live`)는 LIVE guard 때문에 `confirm: true` 필수.
