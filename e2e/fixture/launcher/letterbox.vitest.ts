...
// ---------------------------------------------------------------------------
// #563 document-flow viewport expansion — mechanism contract
// ---------------------------------------------------------------------------
//
// v10 isolation (2026-06-13): setting height on a position:fixed container does
// NOT recalculate the WebKit top-level viewport — a sentinel inside such a box
// still reports isIntersecting:false at y>797. This is why #527 failed.
//
// v10 positive branch: setting height on the DOCUMENT-FLOW ROOT
// (document.documentElement + document.body) DOES recalculate the viewport.
//
// v11 propagation (2026-06-13): when the launcher parent sets
// document.documentElement.style.height = `${screen.height}px` the cross-origin
// child iframe's viewport also expands — no change to dev-app code required.
//
// The Launcher.tsx effect that wires this is DOM-heavy and cannot run in jsdom
// (document.documentElement.style is present but there is no real layout engine).
// The pure precondition — "applyPxCorrection = correctionAttempted && verified
// !== clipped" — is derived from the detectLetterbox result, which IS testable.
// These tests confirm that the condition that triggers the document-flow
// expansion matches the expected device scenarios.
//
// For the actual DOM write (`document.documentElement.style.height = …`) see the
// #563 useEffect in Launcher.tsx — real-device verification is the gate.

describe('#563 applyPxCorrection precondition (document-flow expansion trigger)', () => {
  // applyPxCorrection = correctionAttempted && letterboxVerified !== 'clipped'
  // correctionAttempted = letterboxDetected && shortfallPx > 0
  // This set of tests documents when the trigger fires so a reader can reason
  // about when document.documentElement.style.height is set to screen.height px.

  it('letterbox detected (shortfall > 0) + verified pending → correction is attempted (expansion fires)', () => {
    const verdict = detectLetterbox(
      base({ innerHeight: 797, visualViewportHeight: 797, safeAreaTop: 47 }),
    );
    expect(verdict.detected).toBe(true);
    expect(verdict.shortfallPx).toBeGreaterThan(0);
    // correctionAttempted = true; applyPxCorrection = correctionAttempted && 'pending' !== 'clipped' = true
    const correctionAttempted = verdict.detected && verdict.shortfallPx > 0;
    // Cast to the full union to avoid literal-narrowing false-positive in tsc.
    const letterboxVerified = 'pending' as LetterboxVerification | 'pending';
    const applyPxCorrection = correctionAttempted && letterboxVerified !== 'clipped';
    expect(applyPxCorrection).toBe(true);
  });

  it('sentinel reports visible → expansion stays active', () => {
    const verdict = detectLetterbox(
      base({ innerHeight: 797, visualViewportHeight: 797, safeAreaTop: 47 }),
    );
    const correctionAttempted = verdict.detected && verdict.shortfallPx > 0;
    // Use a union variable so the comparison is not a literal-narrowing no-op.
    const letterboxVerifiedVisible: LetterboxVerification | 'pending' = 'visible' as
      | LetterboxVerification
      | 'pending';
    const applyPxCorrection = correctionAttempted && letterboxVerifiedVisible !== 'clipped';
    expect(applyPxCorrection).toBe(true);
  });

  it('sentinel reports clipped → expansion is dropped (fallback to honest layout)', () => {
    const verdict = detectLetterbox(
      base({ innerHeight: 797, visualViewportHeight: 797, safeAreaTop: 47 }),
    );
    const correctionAttempted = verdict.detected && verdict.shortfallPx > 0;
    const letterboxVerifiedClipped: LetterboxVerification | 'pending' = 'clipped' as
      | LetterboxVerification
      | 'pending';
    const applyPxCorrection = correctionAttempted && letterboxVerifiedClipped !== 'clipped';
    expect(applyPxCorrection).toBe(false);
  });

  it('healthy window (shortfall 0) → expansion never fires regardless of verdict', () => {
    const verdict = detectLetterbox(base()); // healthy, shortfall 0
    expect(verdict.detected).toBe(false);
    const correctionAttempted = verdict.detected && verdict.shortfallPx > 0;
    for (const letterboxVerified of ['pending', 'visible', 'clipped'] as const) {
      const applyPxCorrection = correctionAttempted && letterboxVerified !== 'clipped';
      expect(applyPxCorrection).toBe(false);
    }
  });
