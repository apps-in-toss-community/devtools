 ...
 * slots. #396 makes the router **all-lazy**: NO family boots at startup; every
 ...
 * resolves the same warm `relay` family — the dedicated #378 `relay-external`
 * (mobile) slot is exercised separately below.
 *
...
describe('DualConnectionRouter — relay-first (runDebugServer entry, #354 behavior preserved, all-lazy #396)', () => {
  it('relay first switch (staging) lazy-boots the relay family once', async () => {
  ...
  it('relay → staging twice reuses the warm relay family (no re-boot)', async () => {
    const { router, relay, getRelayBootCount } = makeRouter('relay');
...
describe('DualConnectionRouter — live confirm gate (direction-neutral)', () => {
  it('local-first → live without confirm is rejected and does not boot relay', async () => {
  ...
  it('local-first → live with confirm:true arms the LIVE guard', async () => {
...
describe('DualConnectionRouter — mobile (relay-external) keyed family (#378)', () => {
   ...
   * an intoss relay (`relay-intoss`, served to staging/live) and an external PWA
   * relay (`relay-external`, served to mobile) — so a collision between the two
   * relay slots would be observable (the wrong family / origin would surface).
   * Every slot boots lazily via `bootLazyFor(key)`; nothing boots at construction.
   */
